  ...
  return (
    <div className="flex flex-col gap-5">
      <header className="border-b border-ops-border/20 pb-4">
        <h4 className="text-[15px] font-bold text-ops-text">{t('settings.promptsTitle')}</h4>
        <p className="mt-1 text-[11px] leading-5 text-ops-muted/75">{t('settings.promptsDescription')}</p>
      </header>

      <section className="rounded-xl border border-ops-warning/25 bg-ops-warning/5 p-4">
        <div className="text-xs font-bold text-ops-warning">{t('settings.immutableSafety')}</div>
        <p className="mt-1 text-[11px] leading-5 text-ops-muted">{settings.immutableSafetySummary}</p>
      </section>
      ...
      <div className="flex flex-col gap-4">
        {promptKeys.map((key) => {
          ...
          return (
            <section key={key} className="rounded-xl border border-ops-border/25 bg-ops-deep/35 p-4 sm:p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <h5 className="text-[13px] font-bold text-ops-text">{t(promptTranslationKeys[key].title)}</h5>
                  <p className="mt-1 text-[11px] leading-5 text-ops-muted">{t(promptTranslationKeys[key].description)}</p>
                </div>
                <div className="flex rounded-md border border-ops-border/30 p-0.5">
                  <button type="button" className={`rounded px-2.5 py-1.5 text-[10px] font-semibold ${custom ? 'text-ops-muted' : 'bg-ops-text text-ops-bg'}`} onClick={() => setCustomMode(key, false)}>
                    {t('settings.useDefaultPrompt')}
                  </button>
                  <button type="button" className={`rounded px-2.5 py-1.5 text-[10px] font-semibold ${custom ? 'bg-ops-text text-ops-bg' : 'text-ops-muted'}`} onClick={() => setCustomMode(key, true)}>
              ...
              {custom ? (
                <textarea
                  className="field-control mt-3 min-h-[140px] w-full resize-y font-mono text-[11px] leading-5"
                  value={overrides[key]}
                  maxLength={settings.maxPromptChars}
                  onChange={(event) => onChange({ ...overrides, [key]: event.target.value })}
                />
              ) : (
                <pre className="mt-3 max-h-40 overflow-auto whitespace-pre-wrap rounded-md border border-ops-border/20 bg-ops-bg/35 p-3 text-[11px] leading-5 text-ops-muted">{settings.defaults[key] || t('settings.emptyOrganizationRules')}</pre>
              )}

              <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-[10px] text-ops-muted/70">
              ...
              {previewKey === key ? (
                <pre className="mt-2 max-h-44 overflow-auto whitespace-pre-wrap rounded-md border border-ops-cyan/20 bg-ops-cyan/5 p-3 text-[11px] leading-5 text-ops-text">{effective || t('settings.emptyOrganizationRules')}</pre>
              ) : null}
            </section>
          )
        })}
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-ops-border/20 pt-4">
